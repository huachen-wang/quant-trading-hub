...
async function runMigrations(options: { strict?: boolean } = {}) {
  ...
  try {
    ...
    const complianceCardRepairs: Array<[string, string, string, string, string]> = [
      ['一单一结（武汉小艺）', '一单一结（武汉小艺）', '一次一单·历史资料待核验', '一次一单的执行方案展示；仍可发生连续亏损、滑点和爆仓风险。', '风险边界'],
      ['超级黄金调优 2026', '超级黄金调优 2026', '网格参数方案·高风险', '网格参数与风控边界展示；模拟或历史数据不代表未来结果。', '高风险'],
      ['趋势刷单·EAXAU独家版', '趋势刷单·风险测试版', '趋势马丁·极端行情风险高', '趋势马丁参数方案展示；抗单不等于不会爆仓，不宣称独家或同类唯一。', '高风险'],
      ['暴利引擎 Pro', '高波动引擎 Pro', '高风险策略·不承诺收益', '高波动执行方案展示；仅适合能承受全部本金损失的测试环境。', '高风险'],
    ];
    for (const [oldTitle, title, subtitle, description, badge] of complianceCardRepairs) {
    ...
    const compliancePromoRepairs: Array<[string, string, string, string]> = [
      ['Gold Scalper Pro MT5', '黄金剥头皮 EA 策略资料；交易频率、滑点与历史表现均需以可核验证据为准。', '【策略类型】黄金剥头皮。\n【风险】对点差、滑点、延迟和流动性敏感。\n【证据状态】平台尚未完成独立实盘核验，历史数据不代表未来结果。', '资料待核验'],
      ['Quantum Grid Master', '动态网格参数方案；支持多品种测试，不宣称量子计算或收益保证。', '【策略类型】基于 ATR/波动率参数的动态网格。\n【风险】单边行情可快速累积仓位和回撤。\n【证据状态】表现数据待独立核验。', '高风险'],
      ['Sniper Entry System', '一次一单的趋势入场方案；胜率、盈亏比和回撤需以可核验报告为准。', '【策略类型】多时间框架趋势与关键位确认。\n【风险】信号可失效，止损不消除连续亏损。\n【证据状态】胜率和收益数据待独立核验。', '资料待核验'],
      ['Turbo Scalping Engine', '黄金高频执行方案；对券商规则、成本、滑点和延迟高度敏感。', '【策略类型】Tick 级高频执行。\n【风险】实际成本和流动性可使回测与实盘显著偏离。\n【证据状态】收益与回撤尚未独立核验。', '高风险'],
      ['AI Matrix Trader', '多策略组合与市场状态切换方案；“AI”不代表收益保证。', '【策略类型】多策略组合与状态切换。\n【风险】模型分类可能错误，多策略仍可同时亏损。\n【证据状态】算法实现与表现数据待独立核验。', '资料待核验'],
      ['Neural Trend Follower', '趋势跟踪指标与半自动方案；算法实现、信号质量与交易频率待独立核验。', '【产品类型】趋势跟踪指标+半自动 EA。\n【风险】信号会滞后或失效，不构成投资建议。\n【证据状态】算法与表现尚未独立核验。', '资料待核验'],
    ];
    for (const [title, description, detailContent, promoLabel] of compliancePromoRepairs) {
      const [repairResult] = await connection.query(
    ...
    const paymentAuditMigrations: [string, string][] = [
      ["settlementNetwork", "ALTER TABLE `payments` ADD COLUMN `settlementNetwork` varchar(32) DEFAULT NULL"],
      ["settlementToken", "ALTER TABLE `payments` ADD COLUMN `settlementToken` varchar(16) DEFAULT NULL"],
      ["recipientAddress", "ALTER TABLE `payments` ADD COLUMN `recipientAddress` varchar(255) DEFAULT NULL"],
      ["quotedAmount", "ALTER TABLE `payments` ADD COLUMN `quotedAmount` decimal(20,6) DEFAULT NULL"],
      ["quoteExpiresAt", "ALTER TABLE `payments` ADD COLUMN `quoteExpiresAt` timestamp NULL DEFAULT NULL"],
      ["submittedAt", "ALTER TABLE `payments` ADD COLUMN `submittedAt` timestamp NULL DEFAULT NULL"],
      ["payerWalletAddress", "ALTER TABLE `payments` ADD COLUMN `payerWalletAddress` varchar(255) DEFAULT NULL"],
      ["payerOwnershipAttestedAt", "ALTER TABLE `payments` ADD COLUMN `payerOwnershipAttestedAt` timestamp NULL DEFAULT NULL"],
      ["receivedAmount", "ALTER TABLE `payments` ADD COLUMN `receivedAmount` decimal(20,6) DEFAULT NULL"],
      ["confirmations", "ALTER TABLE `payments` ADD COLUMN `confirmations` int DEFAULT NULL"],
      ["observedNetwork", "ALTER TABLE `payments` ADD COLUMN `observedNetwork` varchar(32) DEFAULT NULL"],
      ["usdtReviewStatus", "ALTER TABLE `payments` ADD COLUMN `usdtReviewStatus` enum('NOT_APPLICABLE','AWAITING_TX','PENDING_REVIEW','MATCHED','UNDERPAID','OVERPAID','WRONG_NETWORK','QUOTE_EXPIRED_RECEIPT','DUPLICATE_TX','REFUND_PENDING','REFUNDED','REJECTED') NOT NULL DEFAULT 'NOT_APPLICABLE'"],
      ["verificationMode", "ALTER TABLE `payments` ADD COLUMN `verificationMode` enum('MANUAL','RPC') DEFAULT NULL"],
      ["reviewedBy", "ALTER TABLE `payments` ADD COLUMN `reviewedBy` int DEFAULT NULL"],
      ["reviewedAt", "ALTER TABLE `payments` ADD COLUMN `reviewedAt` timestamp NULL DEFAULT NULL"],
      ["reviewNote", "ALTER TABLE `payments` ADD COLUMN `reviewNote` text DEFAULT NULL"],
      ["refundAmount", "ALTER TABLE `payments` ADD COLUMN `refundAmount` decimal(20,6) DEFAULT NULL"],
      ["refundNetwork", "ALTER TABLE `payments` ADD COLUMN `refundNetwork` varchar(32) DEFAULT NULL"],
      ["refundTxHash", "ALTER TABLE `payments` ADD COLUMN `refundTxHash` varchar(160) DEFAULT NULL"],
      ["refundRecipientAddress", "ALTER TABLE `payments` ADD COLUMN `refundRecipientAddress` varchar(255) DEFAULT NULL"],
      ["refundVerificationRef", "ALTER TABLE `payments` ADD COLUMN `refundVerificationRef` varchar(120) DEFAULT NULL"],
      ["refundRecipientVerifiedBy", "ALTER TABLE `payments` ADD COLUMN `refundRecipientVerifiedBy` int DEFAULT NULL"],
      ["refundRecipientVerifiedAt", "ALTER TABLE `payments` ADD COLUMN `refundRecipientVerifiedAt` timestamp NULL DEFAULT NULL"],
      ["refundedBy", "ALTER TABLE `payments` ADD COLUMN `refundedBy` int DEFAULT NULL"],
      ["refundedAt", "ALTER TABLE `payments` ADD COLUMN `refundedAt` timestamp NULL DEFAULT NULL"],
    ];
    ...
    const managedFundingColumnMigrations: [string, string][] = [
      ["custodyProvider", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `custodyProvider` enum('MANUAL','BVNK','COBO') NOT NULL DEFAULT 'MANUAL' AFTER `instructionSource`"],
      ["externalProviderRef", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `externalProviderRef` varchar(120) DEFAULT NULL"],
      ["collectionAddressId", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `collectionAddressId` int DEFAULT NULL"],
      ["payerWalletAddress", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payerWalletAddress` varchar(255) DEFAULT NULL"],
      ["payerOwnershipAttestedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payerOwnershipAttestedAt` timestamp NULL DEFAULT NULL"],
      ["customerEligibilityReferenceHash", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `customerEligibilityReferenceHash` varchar(80) DEFAULT NULL"],
      ["customerEligibilityAttestedBy", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `customerEligibilityAttestedBy` int DEFAULT NULL"],
      ["customerEligibilityAttestedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `customerEligibilityAttestedAt` timestamp NULL DEFAULT NULL"],
      ["receivedAmount", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `receivedAmount` decimal(20,6) DEFAULT NULL"],
      ["observedNetwork", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `observedNetwork` varchar(32) DEFAULT NULL"],
      ["creditedAmount", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `creditedAmount` decimal(20,6) DEFAULT NULL"],
      ["confirmations", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `confirmations` int DEFAULT NULL"],
      ["reconciliationResult", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `reconciliationResult` enum('MATCHED','UNDERPAID','OVERPAID','WRONG_NETWORK','LATE_RECEIPT','DUPLICATE_TX','REFUND_PENDING','REFUNDED','MANUAL_REVIEW') DEFAULT NULL"],
      ["screeningStatus", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `screeningStatus` enum('PENDING','CLEARED','HELD','REJECTED') DEFAULT NULL"],
      ["screeningProviderRef", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `screeningProviderRef` varchar(120) DEFAULT NULL"],
      ["complianceNote", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `complianceNote` text DEFAULT NULL"],
      ["clearedBy", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `clearedBy` int DEFAULT NULL"],
      ["clearedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `clearedAt` timestamp NULL DEFAULT NULL"],
      ["payoutAmount", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutAmount` decimal(20,6) DEFAULT NULL"],
      ["payoutNetwork", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutNetwork` varchar(32) DEFAULT NULL"],
      ["payoutDestination", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutDestination` varchar(255) DEFAULT NULL"],
      ["payoutDestinationReferenceHash", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutDestinationReferenceHash` varchar(80) DEFAULT NULL"],
      ["payoutTxHash", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutTxHash` varchar(160) DEFAULT NULL"],
      ["payoutRequestedBy", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutRequestedBy` int DEFAULT NULL"],
      ["payoutRequestedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutRequestedAt` timestamp NULL DEFAULT NULL"],
      ["payoutApprovedBy", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutApprovedBy` int DEFAULT NULL"],
      ["payoutApprovedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutApprovedAt` timestamp NULL DEFAULT NULL"],
      ["payoutSubmittedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `payoutSubmittedAt` timestamp NULL DEFAULT NULL"],
      ["verifiedRefundAddress", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `verifiedRefundAddress` varchar(255) DEFAULT NULL"],
      ["refundAddressVerifiedBy", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `refundAddressVerifiedBy` int DEFAULT NULL"],
      ["refundAddressVerifiedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `refundAddressVerifiedAt` timestamp NULL DEFAULT NULL"],
      ["refundAmount", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `refundAmount` decimal(20,6) DEFAULT NULL"],
      ["refundTxHash", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `refundTxHash` varchar(160) DEFAULT NULL"],
      ["brokerCreditReference", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `brokerCreditReference` varchar(120) DEFAULT NULL"],
      ["exceptionReason", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `exceptionReason` text DEFAULT NULL"],
      ["resolutionNote", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `resolutionNote` text DEFAULT NULL"],
      ["resumeStatus", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `resumeStatus` varchar(32) DEFAULT NULL"],
      ["instructionsIssuedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `instructionsIssuedAt` timestamp NULL DEFAULT NULL"],
      ["instructionsExpireAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `instructionsExpireAt` timestamp NULL DEFAULT NULL"],
      ["submittedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `submittedAt` timestamp NULL DEFAULT NULL"],
      ["receivedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `receivedAt` timestamp NULL DEFAULT NULL"],
      ["reconciledAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `reconciledAt` timestamp NULL DEFAULT NULL"],
      ["creditedAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `creditedAt` timestamp NULL DEFAULT NULL"],
      ["cancelledAt", "ALTER TABLE `managed_broker_funding_intents` ADD COLUMN `cancelledAt` timestamp NULL DEFAULT NULL"],
    ];
    ...
    const approvalMigrations: [string, string][] = [
      ["approvedEntity", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `approvedEntity` varchar(160) DEFAULT NULL"],
      ["approvedRegion", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `approvedRegion` varchar(80) DEFAULT NULL"],
      ["approvedChannelId", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `approvedChannelId` varchar(120) DEFAULT NULL"],
      ["validUntil", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `validUntil` timestamp NULL DEFAULT NULL"],
      ["allowedNetworks", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `allowedNetworks` text DEFAULT NULL"],
      ["minimumAmount", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `minimumAmount` decimal(20,6) DEFAULT NULL"],
      ["maximumAmount", "ALTER TABLE `managed_broker_collection_approvals` ADD COLUMN `maximumAmount` decimal(20,6) DEFAULT NULL"],
    ];
