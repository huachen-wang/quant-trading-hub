...
describe("strategy artwork resolver", () => {
  it("prioritizes grid and hedge behavior over the traded symbol", () => {
    ...
    expect(artwork.image).toBe("/strategy-art/grid-hedge.jpg");
  ...
  it("maps AI, multi-asset and digital-asset strategies to the intelligent artwork", () => {
    ...
    ).toBe("/strategy-art/ai-multiasset.jpg");
  });

  it("uses the gold visual for gold strategies and the trend label elsewhere", () => {
    expect(
      resolveStrategyArtwork({ title: "Gold Trend X", pairs: "XAUUSD" }).label,
    ).toBe("黄金趋势");
    expect(
      resolveStrategyArtwork({ title: "Wave Rider", pairs: "EURUSD" }).label,
    ).toBe("趋势策略");
  });
  ...
  it("selects visual variants deterministically within busy categories", () => {
    ...
    const images = titles.map(
      (title) => resolveStrategyArtwork({ title, pairs: "XAUUSD" }).image,
    );

    expect(new Set(images).size).toBe(2);
    expect(
      resolveStrategyArtwork({ title: titles[0], pairs: "XAUUSD" }).image,
    ).toBe(images[0]);
  });
  ...
  it("uses a memorable strategy keyword instead of repeating the full title", () => {
    ...
    expect(getStrategyShortName("Pro Gold Lion EA v1.31 中文版")).toBe(
      "Gold Lion",
    );
