...
这不是继续开发架构的任务。EAXAU V2 的六策略首页、策略详情、资管/券商两种账户模式、券商高级分仓、
EA 资料库、图文/数据后台、Demo/HTTP/Niubang Provider、数据契约和校验引擎已经实现。Claude Code 的
...
- `/` 与 `/v2-preview` 只展示六个核心策略席位，且只挂载六张可见主图。
- `/v2-preview/strategies/[id]` 已包含净值、指标、持仓、交易及可编辑说明/证据块。
- `/` 已把六策略和“资金 × 风控 × 策略 × 平台 × 模式”选配器放在同一条核心流程中。
- `/v2-preview/allocate` 已支持券商模式下 1 至 3 个平台、推荐方案、手动步进器和桌面拖放快捷方式，并继承首页选择。
- `/v2-preview/accounts` 已覆盖资管模式与券商模式两种只读账户。
- `/v2-preview/ea-library` 保留原 EA 目录、搜索、筛选、咨询和旧链接能力。
- `/admin/v2-content` 已支持新建、编辑、显隐、原子排序、删除、预览和未保存保护。
- `/admin/v2-data` 已支持生成数据样本、编辑接入前历史和设置实盘接管时间。
- `/v2-preview/allocate` 的平台与策略需要先查看速览，再加入方案；详情图库支持缩略图、悬停说明和放大浏览。
...
- 在真实桌面 Chrome 中把兼容策略拖入平台桶，确认先打开规格详情、点击确认后出现成功提示；再验证重复、离线和不兼容策略被拒绝。
