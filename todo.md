# EA策略实盘展示平台 - 任务清单

## 数据库调整
- [x] 删除关注、收藏、评分表
- [x] 保留评论表(作为策略备注)
- [x] 简化策略表字段
- [x] 添加下载和付费相关字段

## API调整
- [x] 移除关注、收藏、评分接口
- [x] 保留评论接口(简化为备注)
- [x] 移除用户发布功能
- [x] 添加下载和购买接口

## UI重构 - 精致简约风格
- [x] 重新设计策略卡片(突出实盘数据)
- [x] 优化配色方案(专业金融风格)
- [x] 简化底部导航(策略列表、我的)
- [x] 移除发布和关注页面
- [ ] 策略详情页(实盘数据为主)
- [ ] 优化排版和间距
- [ ] 添加微动效和过渡动画

## 核心功能
- [ ] 策略列表(实盘数据展示)
- [ ] 策略详情(交易记录、收益曲线)
- [ ] 评论区(策略备注说明)
- [ ] 下载功能
- [ ] 付费购买
- [ ] 联系方式(Telegram/QQ)

## 测试
- [ ] 更新单元测试
- [ ] UI测试


## 管理员后台
- [x] 管理员权限验证中间件
- [x] 策略管理API(创建、编辑、删除)
- [x] 评论管理API(删除)
- [x] 数据统计API
- [x] 管理员布局和权限验证
- [x] 策略列表管理页面
- [x] 策略编辑表单
- [x] 评论管理页面
- [x] 数据统计仪表板
- [x] 个人中心添加管理员入口


## 虚拟数据和优化
- [x] 生成11个虚拟EA策略数据
- [x] 为每个EA添加封面图片
- [x] 添加真实感的策略名称和描述
- [x] 设置合理的价格范围(0-599元)
- [x] 添加虚拟实盘数据(收益率、胜率等)
- [x] 优化Web端展示效果
- [x] 确保移动端和Web端数据同步
- [x] 简化管理后台界面


## UI优化和联系功能
- [x] 策略卡片改为双列网格布局
- [x] 使用简单占位图替换复杂图片(渐变色+emoji)
- [x] 添加"上架EA"联系按钮
- [x] 创建联系方式弹窗组件
- [x] 在弹窗中展示Telegram/QQ/微信等联系方式
- [x] 添加EA代挂合作服务说明
- [x] 优化移动端双列布局响应式


## 策略详情页
- [x] 创建策略详情页面路由
- [x] 展示策略完整信息(标题、描述、平台、交易对等)
- [x] 显示实盘数据(收益率、胜率、回撤、夏普比率)
- [x] 添加下载按钮和联系方式
- [x] 实现评论列表组件
- [x] 实现评论输入框(管理员专用)
- [x] 评论删除功能(管理员)
- [x] 评论时间显示


## AI生成策略内容
- [x] 生成10个专业EA策略封面图
- [x] 编写详细的策略介绍和说明
- [x] 更新策略名称(一次一单、超级黄金、网格大师等)
- [x] 设置真实感的实盘数据
- [x] 更新数据库中的策略信息

## 回测数据模块
- [x] 设计回测数据表结构
- [x] 添加回测数捪eAPI接口
- [x] 创建收益曲线图表组件
- [x] 创建月度收益柱状图组件
- [ ] 创建交易记录列表组件
- [ ] 创建回测统计卡片组件
- [x] 在策略详情页集成回测数据模块
- [x] 生成虚拟回测数据(180天)
- [ ] 管理员后台添加回测数据管理


## 错误修复
- [x] 诊断EA详情页错误原因(victory-native Web端兼容性问题)
- [x] 修复详情页显示问题(改用简化统计展示)
- [x] 测试所有EA详情页


## 用户体验优化
- [x] 移除"我的"页面登录限制(访客模式)
- [x] 简化注册流程(仅需邮箱)
- [x] 移除验证码要求
- [x] 实现主页搜索功能(独立搜索页面)
- [x] 修复MT4/MT5筛选功能(已实现)
- [x] 修复排序功能(最新、最热、收益率)
- [x] 添加管理员数据仪表板(已存在)
- [x] 仪表板展示策略总数、下载量、购买数等统计


## 收藏功能
- [x] 创建收藏管理Hook(useFavorites)
- [x] 使用AsyncStorage本地存储收藏数据
- [x] 在策略卡片添加收藏按钮
- [x] 在策略详情页添加收藏按钮(不需要,卡片已有)
- [x] 创建收藏列表页面
- [x] 在底部导航添加收藏Tab
- [x] 添加收藏/取消收藏动画效果(haptic反馈)
- [x] 测试收藏功能


## UI优化和策略显示修复
- [x] 诊断策略不显示的问题(状态需要设置为published)
- [x] 修复策略列表加载
- [x] 优化顶部间距和布局
- [x] 优化底部导航间距
- [x] 减少页面留白
- [x] 优化策略卡片间距
- [x] 测试所有页面布局


## UI设计优化和策略恢复
- [x] 检查数据库中的策略数据(11个策略存在)
- [x] 恢复AI生成的策略(一次一单、超级黄金等)
- [x] 优化全局字体大小(卡片文字改为text-base)
- [x] 底部导航图标改为emoji(📈/❤️/👤)
- [x] 顶部筛选按钮改为单行横向布局
- [x] 测试所有UI改动(策略正常显示,UI优化生效)


## 新需求 - 用户获取和留言功能

### 策略数据重建
- [x] 重新生成一批EA策略数据(AI生成封面图和详细介绍)
- [x] 清理旧的策略数据
- [x] 添加新的种子数据到数据库(10个新策略)

### 注册登录优化
- [ ] 实现邮箱验证码注册登录系统
- [ ] 移除密码登录,改为验证码登录
- [ ] 添加验证码发送API
- [ ] 添加验证码验证API
- [ ] 创建登录/注册页面

### 访客模式限制
- [ ] 访客可以浏览策略列表
- [ ] 访客点击策略详情时弹出注册引导
- [ ] 注册后才能查看EA详情页
- [ ] 在详情页入口添加登录状态检查

### 上架EA留言表单
- [ ] 创建上架EA表单页面
- [ ] 表单字段:姓名、联系方式(手机/微信/Telegram)、EA描述
- [ ] 展示平台优势(社媒投放、社群覆盖能力)
- [ ] 创建留言提交API
- [ ] 创建管理员查看留言页面

### 匿名留言板
- [ ] 在EA详情页底部添加匿名留言区
- [ ] 用户可以匿名发表评价和问题
- [ ] 创建匿名留言API
- [ ] 显示留言列表
- [ ] 管理员可以删除不当留言

### 当前任务(高优先级)
- [x] 检查首页策略显示问题(服务器重启后正常显示)
- [x] 修复首页数据加载(10个新策略正常显示)
- [x] 实现匿名留言板API
- [x] 添加上架EA表单API
- [x] 在EA详情页添加留言区组件
- [x] 创建合购Tab(底部导航第4个)
- [x] 创建合购数据表(groupBuys)
- [x] 实现合购API接口
- [x] 实现合购发起表单页面
- [x] 实现合购列表页面(显示进度和联系方式)

### 测试
- [ ] 测试首页策略显示
- [ ] 测试匿名留言板功能
- [ ] 测试上架EA留言提交
- [ ] 创建检查点


## 新需求 - UI优化和功能完善

### 邮箱注册登录
- [x] 使用现有OAuth登录系统
- [x] 限制未登录用户查看评论(需登录才能查看)
- [x] 添加登录引导界面

### UI和交互优化
- [x] 所有页面添加返回按钮(发起合购、策略详情等)
- [x] 在各个页面添加emoji优化风格(首页、收藏、合购)
- [ ] 修复底部导航显示不完全问题(需要查看实际问题)

### 收藏功能
- [x] 修复收藏页面不显示内容的问题(添加coverImage字段)
- [x] 测试收藏和取消收藏功能

### 管理后台
- [x] 优化管理后台界面(添加emoji和新菜单)
- [x] 添加合购申请管理入口
- [x] 添加留言管理入口
- [ ] 提供后台访问说明


## Web应用优化

### 响应式布局优化
- [ ] 在所有主要页面应用WebContainer组件
- [ ] 桌面端限制最大宽度(1280px)并居中
- [ ] 优化移动端全屏显示
- [ ] 优化平板端中等宽度显示

### 移除移动端特性
- [ ] 移除Expo Go相关提示和二维码
- [ ] 隐藏原生应用下载提示
- [ ] 优化Web端专属交互

### SEO和元数据
- [ ] 配置页面标题和描述
- [ ] 添加关键词meta标签
- [ ] 优化Open Graph标签
- [ ] 添加favicon和app图标

### 性能优化
- [ ] 配置生产环境打包
- [ ] 优化图片加载
- [ ] 实现代码分割
- [ ] 添加加载状态

### 测试
- [ ] 测试手机浏览器显示
- [ ] 测试桌面浏览器显示
- [ ] 测试平板浏览器显示
- [ ] 创建最终检查点


## 桌面端优化和域名部署

### 桌面端布局优化
- [x] 内容区域限制最大宽度1280px并居中(CSS媒体查询)
- [x] 策略列表改为3-4列布局(桌面端4列,平板3列,移动端2列)
- [x] 添加hover效果(CSS)
- [x] 创建响应式Hook自动检测屏幕尺寸
- [x] 优化桌面端样式(desktop.css)

### 域名配置和部署
- [x] 提供部署指南(Vercel/Netlify/自建服务器)
- [x] 域名DNS配置说明
- [x] SSL证书配置指南
- [ ] 实际部署到生产环境(需用户操作)
- [ ] 配置自定义域名(需用户提供域名)

### 测试
- [ ] 测试桌面端显示(Chrome/Safari/Firefox)
- [ ] 测试移动端显示(保持原样)
- [ ] 测试平板端显示
- [ ] 创建最终检查点


## Vercel部署显示源码问题修复

### 问题
- [ ] Vercel部署后显示源码而不是渲染的网页
- [ ] 需要正确配置Expo Web构建

### 解决方案
- [ ] 创建正确的vercel.json配置
- [ ] 配置正确的构建命令和输出目录
- [ ] 推送到GitHub并触发重新部署
- [ ] 验证部署成功


## Railway部署配置

### 问题
- [ ] Railway部署失败:Metro bundler依赖错误
- [ ] 需要创建Railway专用配置文件

### 解决方案
- [ ] 创建railway.json配置文件
- [ ] 创建Dockerfile优化构建流程
- [ ] 创建.dockerignore文件
- [ ] 推送到GitHub触发重新部署
- [ ] 验证Railway部署成功
- [ ] 获取公开访问域名

### 当前进度
- [x] 创建railway.json配置文件
- [x] 创建Dockerfile优化构建流程
- [x] 创建.dockerignore文件
- [x] 推送到GitHub触发重新部署
- [x] Railway容器成功启动
- [x] 修复pnpm-lock.yaml文件并重新推送
- [x] 修复启动命令和端口配置
- [x] 修复Expo Web构建问题
- [x] 修复Web服务器监听地址(0.0.0.0)
- [ ] 配置OAUTH_SERVER_URL环境变量
- [ ] 获取公开访问域名并测试

### 端口冲突问题修复
- [x] 修复API和Web服务器端口冲突(EADDRINUSE错误)
- [x] 合并API和Web服务器为单一服务
- [x] 让API服务器同时提供静态文件服务
- [x] 测试单一服务器部署
- [x] 简化为纯API服务器部署(移除Web构建)
- [ ] 验证API服务器部署成功


## Web应用完整部署(方案1)

### 问题诊断
- [ ] 查看之前的Expo Web构建错误日志
- [ ] 分析react-native-css-interop依赖问题
- [ ] 检查NativeWind在Web端的兼容性

### 构建错误修复
- [ ] 修复模块导入错误
- [ ] 测试本地Web构建(pnpm expo export --platform web)
- [ ] 验证dist目录生成成功

### Dockerfile更新
- [ ] 恢复Web应用构建步骤
- [ ] 配置静态文件服务
- [ ] 测试Docker构建流程

### 部署验证
- [ ] 推送到GitHub触发Railway重新部署
- [ ] 验证浏览器可以访问完整应用
- [ ] 测试所有页面功能


## 域名配置和后台管理

### 域名配置
- [x] 配置Cloudflare CDN
- [x] 绑定自定义域名(eaxau.com, www.eaxau.com)

### 数据库问题修复
- [ ] 检查Railway数据库连接状态
- [ ] 诊断策略数据不显示的原因
- [ ] 修复数据库数据加载问题

### 后台管理配置
- [ ] 提供后台管理入口说明
- [ ] 配置管理员账号
- [ ] 测试后台管理功能

### 品牌信息更新
- [ ] 修改网站名称为"量化军火库"
- [ ] 更新页面标题和meta信息
- [ ] 更新导航栏和页脚品牌信息


## 代码推送和管理后台开发

### 代码推送到Railway
- [x] 推送所有修复到GitHub仓库
- [x] 触发Railway自动部署
- [ ] 验证部署成功

### 数据库策略保留
- [x] 保留现有10个策略数据(不运行清空脚本)
- [ ] 确认生产数据库数据完整性

### 管理后台界面开发
- [x] 创建策略管理页面(列表、新增、编辑、删除)
- [x] 实现策略表单组件
- [ ] 添加图片上传功能
- [x] 实现策略删除确认对话框
- [x] 优化管理后台UI/UX
- [x] 创建管理后台使用文档(ADMIN_GUIDE.md)
- [x] 创建设置管理员脚本(scripts/set-admin.ts)
- [ ] 测试所有管理功能


## Railway环境变量和管理后台入口修复

### Railway环境变量配置
- [ ] 修复OAUTH_SERVER_URL配置错误
- [ ] 在Railway控制台设置OAUTH_SERVER_URL环境变量
- [ ] 重新部署并验证

### 管理后台独立入口
- [x] 创建/admin路由的直接访问入口
- [x] 添加管理员登录页面(/admin/login)
- [x] 优化管理后台访问流程(支持简单登录)
- [ ] 测试独立入口功能


## web-build目录缺失修复
- [x] 恢复web-build目录（从Git历史commit 8a1ec1c恢复）
- [x] 推送到GitHub触发Railway重新部署
- [ ] 验证eaxau.com正常显示


## 电脑端布局优化
- [ ] 修复卡片右侧被截断问题（第三列显示不全）
- [ ] 优化卡片网格布局，使列数自适应屏幕宽度
- [ ] 统一卡片宽度和间距
- [ ] 提升整体视觉效果，使布局更规整美观
- [x] 重新构建web-build并推送部署

## 新需求：后台登录修复、合购优化、动态页面
- [x] 修复后台登录按钮点击无反应
- [x] 优化合购界面卡片样式（更美观）
- [x] 合购点击显示联系方式页面
- [x] 新增"动态"Tab页面（朋友圈风格信息流）
- [x] 动态页面支持公众号信息、最新更新、行业资讯等卡片
- [x] 重新构建web-build并推送部署到Railway
## Bug修复：admin登录 + 部署UI不更新
- [ ] 修复admin登录按钮在web端点击无反应（AsyncStorage在web端的兼容性问题）
- [ ] 确保合购页面新UI（渐变卡片+联系方式弹窗）正确部署
- [ ] 确保动态Tab页面出现在导航中
- [ ] 重新构建web-build并推送到GitHub/Railway

## 新需求：删除收藏/我的Tab + 新增订阅页面 + 修复admin登录
- [x] 彻底修复admin登录按钮在web端点击无反应（EventEmitter通知layout更新状态）
- [x] 删除"收藏"Tab页面（隐藏）
- [x] 删除"我的"Tab页面（隐藏）
- [x] 新增"订阅"Tab页面（邮箱订阅功能）
- [x] 订阅页面展示技术支持与合作信息
- [x] 订阅页面内容支持后台自定义
- [x] 后台管理增加订阅内容管理功能
- [x] 重新构建web-build并推送部署

## 新需求：详情页优化+上架EA后台配置
- [x] 确认之前的修改（评论匿名、订阅弹窗、通知栏）已正确部署到线上
- [x] 策略详情页电脑端布局优化（去除臃肿感）
- [x] 移除夏普比率字段
- [x] 上架EA联系方式弹窗内容改为后台可配置（不再硬编码）
- [x] 重新构建web-build并推送部署

## 紧急修复：Admin后台所有API 403错误
- [ ] 分析admin认证架构（adminProcedure依赖protectedProcedure→OAuth session）
- [ ] 创建独立admin认证中间件（基于密码header，不依赖OAuth）
- [ ] 修改前端admin页面在所有API调用中发送admin认证header
- [ ] 测试策略管理CRUD（列表、创建、编辑、删除）
- [ ] 测试评论审核功能
- [ ] 测试通知管理功能
- [ ] 测试联系方式设置功能
- [ ] 测试订阅页面管理功能
- [ ] 重新构建web-build并推送部署

## Admin后台全面修复（数据库连接+API+前端）
- [x] 诊断数据库查询失败根本原因（.env.development中旧DATABASE_URL覆盖系统变量）
- [x] 修复load-env.ts不再override系统环境变量
- [x] 修复admin-api.ts的superjson数据解析（data.result.data.json）
- [x] 修复admin-api.ts的错误消息提取（data.error.json.message）
- [x] 添加合购管理admin CRUD路由（create/update/delete）
- [x] 添加上架申请管理admin路由（delete）
- [x] 创建合购管理admin页面（group-buys.tsx）
- [x] 创建上架申请管理admin页面（listings.tsx）
- [x] 添加合购管理和上架申请的admin-api函数
- [x] 将.env文件从git跟踪中移除并添加到.gitignore
- [x] 清理调试日志和测试文件
- [x] 重新构建web-build
- [x] 推送到GitHub触发Railway部署
- [ ] 验证生产环境admin后台正常工作

## 前台+后台bug修复（用户反馈 2026-02-11）
- [x] 修复发起合购“提交申请”按钮点击无反应
- [x] 修复邮箱订阅输入框输入法弹出后跳出的bug
- [x] 修复后台联系方式设置保存失败（“保存失败，请重试”）
- [x] 修复后台创建策略失败（“Load failed”）
- [x] 修复后台数据不显示问题（策略列表等）
- [x] 重新构建web-build并推送部署

## UI优化：滚动、导航、空状态、动画（2026-02-11）
- [x] 修复四个主页面不滚动问题（创建+html.tsx自定义HTML模板移除overflow:hidden）
- [x] 修复底部导航栏文字被手机浏览器遮挡（增加web端bottomPadding到24px）
- [x] 搜索页面空状态内容居中
- [x] 空列表添加友好提示（EmptyState组件+各页面空状态优化）
- [x] 添加卡片入场动画（AnimatedListItem组件淡入+上滑）
- [x] 添加按钮点击反馈动画（策略卡片按压缩放+haptic反馈）
- [x] 添加页面过渡动画（Modal弹出缩放动画）
- [x] 添加列表项渐入动画（moments/group-buy列表项依次渐入）
- [x] 重新构建web-build并推送部署


## GPT Debug报告修复（2026-02-11）

### P0 安全问题
- [x] 移除硬编码管理员密码（server/_core/context.ts中的"admin123"）
- [x] 移除前端硬编码密码（lib/admin-api.ts）
- [x] 实现服务端签发的管理员token认证（JWT + jose）
- [x] 管理员密码从环境变量读取（ADMIN_EMAIL/ADMIN_PASSWORD）
- [x] 后台登录改为服务端验证（app/admin/login.tsx）

### P0 稳定性问题
- [x] 修夏API端口漂移问题（移除findAvailablePort函数）
- [x] 开发环境固定端口失败即退出（Fail Fast）
- [x] 确保前后端端口一致性

### P1 类型安全问题
- [x] 修夏app/(tabs)/subscribe.tsx的implicit any
- [x] 修夏app/strategy/[id].tsx的implicit any
- [x] 修夏tests/admin.test.ts的implicit any
- [x] 修夏tests/simplified-api.test.ts的implicit any
- [x] 修夏tests/ux-improvements.test.ts的implicit any

### P1 测试可靠性问题
- [ ] 为测试环境增加数据库隔离层（sqlite/in-memory）
- [ ] 修复测试用例ECONNREFUSED错误
- [ ] 添加JWT_SECRET环境变量

### P2 权限一致性问题
- [x] 修复管理员删除评论权限（admin.comments.delete已使用adminProcedure）
- [x] 统一管理员删除逻辑（调用deleteCommentByAdmin）

### 部署
- [x] 测试所有修复（TypeScript检查通过）
- [x] 重新构建web-build
- [ ] 推送到GitHub/Railway（需用户手动推送或配置CI/CD）


## 管理后台数据添加/编辑问题修复（2026-02-11）

### 问题描述
- [ ] 管理后台无法添加/编辑任何数据（策略、动态、通知等）
- [ ] 数据读取和保存路径不一致（编辑页走公共接口，保存走admin接口）
- [ ] 公共接口会增加浏览量等副作用

### 修复方案
- [x] 创建admin.strategies.detail接口（不增加浏览量）
- [x] 管理后台编辑页改为调用admin.strategies.detail
- [x] 添加管理后台列表页自动刷新机制（useFocusEffect）
- [x] 测试后台添加/编辑策略功能（所有测试通过）
- [x] 修夏createStrategy返回插入的策略对象
- [ ] 测试后台添加/编辑动态功能
- [ ] 测试后台添加/编辑通知功能


## 管理后台"Failed to fetch"错误修复（2026-02-11）

### 问题描述
- [ ] 后台添加策略时显示"Failed to fetch"错误
- [ ] 后台列表页无法显示前台已有的策略数据
- [ ] 可能原因：JWT认证失败、API地址错误、CORS问题

### 修复方案
- [ ] 检查管理员是否成功登录并获取token
- [ ] 检查admin-api.ts的getAdminToken逻辑
- [ ] 检查API地址配置（开发环境 vs 生产环境）
- [ ] 检查server CORS配置是否允许admin请求
- [ ] 添加详细的错误日志和调试信息


## 彻底修复管理后台“Failed to fetch”（2026-02-11 第二轮）
- [x] 排查生产环境API是否可访问（API路由正常，返回数据）
- [x] 排查admin登录和token认证链路（发现API地址编译为开发服务器）
- [x] 排查admin-api.ts的fetch调用（getApiBaseUrl返回开发服务器地址）
- [x] 确保web-build包含最新代码（使用--clear标志重新构建，apiBaseUrl已编译为https://eaxau.com）
- [x] 推送到GitHub触发Railway部署
- [ ] 验证生产环境admin功能正常（等待Railway部署完成）


## Bug修复：crypto is not defined + 合购卡片布局 + 虚拟值功能（2026-02-11）
- [ ] 修夏admin登录"crypto is not defined"错误（uuid库在旧浏览器中调用crypto）
- [x] 修复合购页面电脑端卡片太窄，文字被截断
- [x] 订阅数量支持自定义虚拟值（虚拟值+实际值）
- [x] 下载量支持设置默认虚拟值
- [x] 重新构建web-build并推送部署


## 紧急修复：admin登录crypto错误（2026-02-11）
- [x] 添加crypto polyfill到dist/index.html
- [x] 重新构建web-build
- [x] 推送到GitHub触发Railway部署
- [ ] 验证admin登录功能正常


## 紧急简化：移除admin登录验证（2026-02-11）
- [x] 修改/admin路由直接跳转到后台首页
- [x] 移除adminProcedure的token验证要求
- [x] 移除admin-api.ts中的token header
- [ ] 构建web-build并推送部署
- [ ] 验证后台可直接访问
