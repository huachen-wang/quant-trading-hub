      ...
      <BlockFrame heading={block.heading} icon="fact-check">
        <View style={styles.rows}>
          {block.items.map((item) => {
            ...
            const label =
              item.status === "VERIFIED"
                ? text("已核验", "Verified", "تم التحقق")
                : item.status === "DEMO"
                  ? text("模拟占位", "Demo placeholder", "عنصر تجريبي")
                  : text("待补充", "Pending", "قيد الاستكمال");
            return (
...
function MediaGalleryBlock({
  ...
  block: Extract<ContentBlock, { type: "media_gallery" }>;
}) {
  ...
  const move = (direction: -1 | 1) => {
    ...
    setSelectedIndex(
      (selectedIndex + direction + block.items.length) % block.items.length,
    );
  };
    ...
    <>
            ...
            return (
              <Pressable
                ...
                accessibilityLabel={text(
                  ...
                  `عرض الصورة: ${item.title}`,
                )}
                onHoverIn={() => Platform.OS === "web" && setHovered(item.id)}
                onHoverOut={() =>
                  Platform.OS === "web" && setHovered(undefined)
                }
              ...
              >
                ...
                <View
                  style={[
                    ...
                    (active || isMobile) && styles.galleryCaptionActive,
                  ]}
                >
                  <View style={styles.galleryCaptionCopy}>
                    ...
                    {(active || isMobile) && item.caption ? (
                      <Text style={styles.galleryDetail} numberOfLines={2}>
                        {item.caption}
                      </Text>
                    ) : null}
                  </View>
      ...
      >
        <View style={styles.lightboxBackdrop}>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={text("关闭图片", "Close image", "إغلاق الصورة")}
              ...
              <View style={styles.lightboxFooter}>
                ...
                <View style={styles.lightboxCopy}>
                  ...
                  <Text style={styles.lightboxCount}>
                    {(selectedIndex ?? 0) + 1} / {block.items.length}
                  </Text>
                </View>
                <Pressable
                  accessibilityRole="button"
...
function BlockFrame({
  ...
  return (
    <View style={[styles.block, isMobile && styles.blockMobile]}>
      <View
        style={[styles.blockHeading, isMobile && styles.blockHeadingMobile]}
      >
...
const styles = StyleSheet.create({
  ...
  timelineLine: {
    ...
    width: 1,
    marginVertical: 4,
    backgroundColor: V2.borderStrong,
  },
  ...
  riskBlock: {
    padding: 18,
    borderWidth: 1,
    borderColor: "rgba(231,183,95,0.38)",
    borderRadius: 5,
    backgroundColor: "rgba(231,183,95,0.06)",
    flexDirection: "row",
    gap: 13,
  },
